{
  description = "CIY Kube Scheduler";
  ...
  in
    {
      ...
      in rec {
        ciy-scheduler = pkgs.buildGo121Module rec {
          pname = "ciy-scheduler";
          version = ciySchedulerVerions;
          ...
          ldflags = ["-s" "-w" "-X k8s.io/component-base/version.gitVersion=v${version}"];
        };
    ...
    (system: let
      ...
      # Add entry to build a docker image with ciy-scheduler
      # caveat: only works on Linux
      #
      # Usage:
      # nix build .#ciy-scheduler-docker
      # docker load < result
      ciy-scheduler-docker = pkgs.dockerTools.buildLayeredImage {
        name = "ciy-scheduler";
        tag = ciySchedulerVerions;
        contents = [pkgs.ciy-scheduler];
        config.Entrypoint = [(pkgs.ciy-scheduler + "/bin/ciy-scheduler")];
      };
    in rec {
      # `nix develop`
      ...
      # `nix build`
      packages = with pkgs; {
        inherit ciy-scheduler;
        inherit ciy-scheduler-docker;
      };
      defaultPackage = pkgs.ciy-scheduler;

      # `nix run`
      apps.ciy-scheduler = flake-utils.lib.mkApp {
        drv = packages.ciy-scheduler;
      };
      apps.default = apps.ciy-scheduler;
